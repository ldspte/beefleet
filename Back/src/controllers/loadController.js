...
const getLoads = async () => {
  const results = await db.query(`SELECT * FROM cargas`);
  return results;
}
...
const createLoad = async (descripcion, peso, foto_carga, fecha_inicio, fecha_fin, vehiculo, cliente, conductor) => {
  const result = await db.query(
    `INSERT INTO cargas (descripcion, peso, foto_carga, fecha_inicio, fecha_fin, vehiculo, cliente, conductor) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
const updateLoad = async (id_carga, descripcion, peso, foto_carga, fecha_inicio, fecha_fin, vehiculo, cliente, conductor) => {
  const result = await db.query(
    `UPDATE cargas 
     SET descripcion = ?, peso = ?, foto_carga = ?, fecha_inicio = ?, fecha_fin = ?, vehiculo = ?, cliente = ?, conductor = ? 
     WHERE id_carga = ?`,
